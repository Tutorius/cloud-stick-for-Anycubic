...
static const unsigned char PROGMEM image_wifi_bits[] = {0x01,0xf0,0x00,0x06,0x0c,0x00,0x18,0x03,0x00,0x21,0xf0,0x80,0x46,0x0c,0x40,0x88,0x02,0x20,0x10,0xe1,0x00,0x23,0x18,0x80,0x04,0x04,0x00,0x08,0x42,0x00,0x01,0xb0,0x00,0x02,0x08,0x00,0x00,0x40,0x00,0x00,0xa0,0x00,0x00,0x40,0x00};

// Dashboard
static const unsigned char PROGMEM image_cloud_1_bits[] = {0x07,0xc0,0x00,0x08,0x20,0x00,0x10,0x10,0x00,0x30,0x08,0x00,0x40,0x0e,0x00,0x80,0x01,0x00,0x80,0x00,0x80,0x40,0x00,0x80,0x3f,0xff,0x00};
...
static const unsigned char PROGMEM image_internet_bits[] = {0x03,0xc0,0x0d,0xb0,0x32,0x4c,0x24,0x24,0x44,0x22,0x7f,0xfe,0x88,0x11,0x88,0x11,0x88,0x11,0x88,0x11,0x7f,0xfe,0x44,0x22,0x24,0x24,0x32,0x4c,0x0d,0xb0,0x03,0xc0};
...
static const unsigned char PROGMEM image_refresh_1_bits[] = {0x01,0xe0,0x7c,0x38,0x1c,0x0c,0x3c,0x0c,0x74,0x06,0x64,0x06,0x60,0x06,0x60,0x26,0x60,0x2e,0x30,0x3c,0x30,0x38,0x18,0x3e,0x07,0x00};
...
static DashboardState prev_state = {-1, false, false, false, false, 0, 0, 0, 0, {0}};
...
void lcd_dashboard_update(const DashboardState& state) {
    ...
    if (state.wifi_signal != prev_state.wifi_signal) {
        ...
        if (state.wifi_signal == 3) tft.drawBitmap(2, 2, image_wifi_full_bits, 19, 16, 0xFFFF);
        else if (state.wifi_signal == 2) tft.drawBitmap(2, 2, image_wifi_50_bits, 19, 16, 0xFFFF);
        else if (state.wifi_signal == 1) tft.drawBitmap(2, 2, image_wifi_2_bars_bits, 19, 16, 0xFFFF);
    ...
    if (state.upload_speed_kbps != prev_state.upload_speed_kbps) {
        ...
        tft.drawString("U:         ", 26, 24); // clear old text by padding with spaces
    ...
    if (strcmp(state.last_action, prev_state.last_action) != 0) {
        ...
        strncpy(prev_state.last_action, state.last_action, sizeof(prev_state.last_action));
    }
}
